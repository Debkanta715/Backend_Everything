...
//how we know that the user are login or not login if user have a token then user are login and if user have no token then user are not login 


and user are gate the token when they are registerd means login 
